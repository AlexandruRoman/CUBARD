...
public class CaracterMovement_Tester : MonoBehaviour {
	...
	public GameObject GNegru, Tata, Cealalta;
	...
	void Awake()
	{
		...
		transform.position = StartPoint.transform.position + new Vector3(0, 0.8f, 0);
	...
	IEnumerator Scor()
	{
		...
		string cont = PlayerPrefs.GetString("CONT");
		form.AddField("cont", cont);
		print (PlayerPrefs.GetString("CONT"));
		n = PlayerPrefs.GetInt("SCOR") + script.scor;

		PlayerPrefs.SetInt("SCOR", n);

		form.AddField("scor", n);

		na = PlayerPrefs.GetInt("NIVELASTA");


		if(na < PlayerPrefs.GetInt("MAX") && na == PlayerPrefs.GetInt("NIVEL"))
		{
			na++;
			PlayerPrefs.SetInt("NIVEL", na);
		}

		form.AddField("nivel", na);
